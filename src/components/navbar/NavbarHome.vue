<template>
    <nav class="fixed left-0 right-0 top-0 z-[800] flex w-full items-center px-4 md:px-14 lg:px-16"
        :class="!is_scrolling ? 'backdrop-blur-xl bg-white/[0.5]' : 'bg-white'">
        <div class="py-4 md:py-8">
            <router-link :to="{ name: 'HomeView' }">
                    <img :src="getLocalGeneral?.icon?.navbar" alt="navbar_logo" class="sm:block w-[100px] lg:w-[140px]">
            </router-link>
          
            <!-- <img :src="(imgUrl ? imgUrl : getLocalGeneral?.icon?.footer)" alt="navbar_logo"
                class="block sm:hidden w-[100px] lg:w-[140px]">-->
        </div>
        <ul id="is_expand"
...
export default {
    ...
    data() {
        return {
            ...
            getLocalGeneral: {},
            is_scrolling: false,
            active_scroll: 'backdrop-blur-xl',
    ...
    methods: {
        ...
        onScroll() {
            if (window.pageYOffset > 0) {
                ...
                this.imgUrl = this.getLocalGeneral?.icon?.navbar
                // this.active_scroll = 'bg-white'
            } else {
                this.is_scrolling = false
                this.imgUrl = this.getLocalGeneral?.icon?.footer
