    ...
    <div class="static">
        ...
        <section class="relative z-10 bg-gal-grey-light px-2 pb-10 pt-20 font-roboto before:absolute before:left-0 before:top-0 before:z-[-1] before:block before:h-72 before:w-full before:bg-primary before:content-[''] md:px-0 md:pb-20 md:pt-32 md:before:h-80 lg:before:h-96">
            <div class="container mx-auto">
                <div class="gal-title-2 uppercase">{{ getLocale == 'EN' ? TermCondition.page_en : TermCondition.pages }}</div>
                        ...
                        <div v-for="item in TermCondition.contact">
                            ...
                            <template v-for="item_desc in item.description">
                                ...
                                <ul class="gal-items ml-8 list-disc">
                                    <li v-for="item_list in item_desc.list">{{ getLocale == 'EN' ? item_list.name_en : item_list.name }}</li>
                                </ul>
                            </template>
                        </div>
                    </div>
    ...
    methods: {
        getData () {
            if (localStorage.getItem('termspreview')) {
                this.TermCondition = JSON.parse(localStorage.getItem('termspreview'))
