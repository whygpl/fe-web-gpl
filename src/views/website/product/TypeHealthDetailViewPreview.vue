        ...
        <section class="w-full py-20 md:py-32">
            <div class="container mx-auto p-4 md:p-0">
                <div class="relative flex self-center">
                    ...
                    <div class="w-full">
                        <div class="font-roboto text-xl font-[900] uppercase text-gal-blue-light md:text-3xl"> {{ getLocale == 'EN' ? ProductDetail.category.type_title_en : ProductDetail.category.type_title }}</div>
                        <!-- breadcrumbs -->
                        <div class="breadcrumbs mt-4 font-roboto text-sm">
                            <ul>
                                <li v-for="(item, index) in breadcrumbs" :key="index">
                                    <router-link :to="item.link" class="flex items-center gap-2" :class="(index + 1) == breadcrumbs.length ? 'text-primary font-[700]' : ''">{{ getLocale == 'EN' ? item.name_en : item.name  }}</router-link>
                                </li>
                            </ul>
                        </div>
                        <!-- product detail -->
                        <div class="mt-4 grid grid-cols-1 gap-4 md:mt-6 md:grid-cols-4 md:gap-10">

                            <!-- product image -->
                            <div class="">
                                <div class="group relative grid aspect-square w-full place-items-center bg-white overflow-auto touch-pan-y">
                                    <!-- <div class="cursor-pointer h-full w-full object-cover object-center"></div> -->
                                    <template v-if="image_url_detail[active_preview].imgvid == 'img'" >
                                        <div id="zoompicture" :style="{backgroundImage: `url(${image_url_detail[active_preview].image_url})`}" class="zoompicture cursor-pointer h-full w-full object-cover object-center">
                                            <img :src="image_url_detail[active_preview].image_url" alt="" class="zoompicture h-full w-full object-cover object-center">
                                        </div>
                                    </template>
                                    <template v-else>
                                        <div id="zoompicture" style="display:none;"></div>
                                        <template v-if="image_url_detail[active_preview].imgvid == 'vid'" >
                                    ...
                                    <label for="expand_modal" class="btn btn-circle btn-sm absolute right-2 top-2 hidden place-items-center group-hover:grid">
                                    ...
                                    <div class="modal modal-bottom md:modal-middle">
                                        ...
                                        <div class="modal-box p-0 py-6 overflow-hidde md:w-[80%]">
                                            <div class="flex items-start justify-between px-6 ">
                                                ...
                                                <label for="expand_modal" class="btn btn-circle btn-ghost btn-sm font-[900] leading-[100%]">✕</label>
                                            </div>
                                            ...
                                            <div class=" gap-6 px-6 md:grid-cols-3">
                                                <div class="col-span-2 grid grid-cols-5 place-items-center">
                                                    ...
                                                    <div class="col-span-3">
                                                        ...
                                                        <template v-if="image_url_detail[active_preview].imgvid == 'img'">
                                                            <img :src="image_url_detail[active_preview].image_url" alt="" class="zoompicturemodal place-items-center ">
                                                        <!-- </div> -->
                                                ...
                                                <div class="hidden">
                                                    <h3 class="gal-items font-[700]">{{ getLocale == 'EN' ? 'Product Picture' : 'Gambar Barang' }}</h3>
                                                    <div class="mt-4 grid grid-cols-4 gap-4 md:grid-cols-3">
                                                        <div @click="getSelectedImage(index)" v-for="(photo, index) in image_url_detail" class="aspect-square w-full cursor-pointer rounded border-2 border-gal-blue-lighter bg-white p-1" :class="{'border-2 border-gal-blue-light' : index == active_preview}">
                                                            <img :src="photo.image_url" :alt="'image-' + (index + 1)" class="h-full w-full object-cover object-center">
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                ...
                                <div class="relative mt-2 grid md:mt-4">
                                    <div id="image_list" class="flex w-full gap-2 overflow-hidden scroll-smooth md:gap-4">
                                        <div @click="getSelectedImage(index)" v-for="(photo, index) in image_url_detail" class="aspect-square w-1/4 flex-none cursor-pointer rounded border-2 border-gal-blue-lighter bg-white p-1" 
                                            :class="{'border-2 border-gal-blue-light' : index == active_preview}"
                                            >
                                            <template v-if="photo.imgvid == 'img'">
                                                <img :src="photo.image_url" :alt="'image-' + (index + 1)" class="h-full w-full object-cover object-center">
                                            </template>
                                            <template v-else>
                                    ...
                                    <button @click="MoveLeft" class="btn btn-sm absolute left-0 place-self-center" :class="scroll_position != 0 ? 'block' : 'hidden'">
                                        <font-awesome-icon icon="fa-solid fa-chevron-left" />
                                    </button>
                                    <button @click="MoveRight" class="btn btn-sm absolute right-0 place-self-center" :class="image_url_detail.length > 3 ? 'block' : 'hidden'">
                                        <font-awesome-icon icon="fa-solid fa-chevron-right" />
                                    </button>
                                </div>
                            </div>
                            ...
                            <div class="mt-4 font-roboto md:col-span-3 md:mt-0">
                                <div>
                                    ...
                                    <div class="flex items-center">
                                        <div class="relative-blockquote before:bg-[#34b6ff]"></div><div class="text-base font-[900] uppercase text-gal-blue-light md:text-lg"><template v-if="ProductDetail.category.type_id == 1">{{ getLocale == 'EN' ? ProductDetail.group.title_en : ProductDetail.group.title }}</template><template v-else>{{ getLocale == 'EN' ? ProductDetail.category.title_en : ProductDetail.category.title }}</template></div>
                                    </div>
                                    <div class="gal-title text-4xl mt-2 md:mt-4">{{ getLocale == 'EN' ? ProductDetail.name_en : ProductDetail.name }}</div>
                                    <div class="flex">
                                        &nbsp;
                                    ...
                                    <div class="mt-4 font-roboto">
                                        <div class="font-[800] text-gal-grey-darker">{{ getLocale == 'EN' ? 'Description' : 'Deskripsi' }}</div>
                                        <div class="gal-items mt-1">{{ getLocale == 'EN' ? ProductDetail.description_en : ProductDetail.description }}</div>
                                    </div>
                                    <div class="mt-4 font-roboto">
                                        <div class="font-[800] text-gal-grey-darker">{{ getLocale == 'EN' ? 'How to use' : 'Cara Pakai' }}</div>
                                        <ul class="ml-8 mt-1 list-disc">
                                            <li v-for="list in ProductDetail.htu" class="gal-items">{{ getLocale == 'EN' ? list.name_en : list.name }}</li>
                                        </ul>
                                    </div>
                                    <div class="mt-4 font-roboto">
                                        <div class="font-[800] text-gal-grey-darker">{{ getLocale == 'EN' ? 'Compotition' : 'Kandungan' }}</div>
                                        <div class="gal-items mt-1">{{ getLocale == 'EN' ? ProductDetail.compotition_en : ProductDetail.compotition }}</div>
                                    </div>
                                </div>
                                <div class="mt-6 flex flex-wrap gap-2 md:mt-10 md:gap-5">
                                    <template v-if="ProductDetail.microsite">
                                        <a :href="ProductDetail.micrositeurl" target="_blank" class="btn-outline-secondary btn">
                                            <span>{{ getLocale == 'EN' ? 'Visit Microsite' : 'Kunjungi Microsite' }}</span>
                                            <font-awesome-icon icon="fa-solid fa-arrow-right-long"/>
                                        </a>
                                    </template>
                                    <template v-if="ProductDetail.gstore">
                                        <a :href="ProductDetail.gstoreurl" target="_blank" class="btn-outline-secondary btn">
                                            <span>{{ getLocale == 'EN' ? 'Get in G-Store' : 'Dapatkan di G-Store' }}</span>
                                            <font-awesome-icon icon="fa-solid fa-arrow-right-long"/>
                                        </a>
                                    </template>
                                  
                                    <template v-if="ProductDetail.chat">
                                    <a :href="ProductDetail.chatvalue" target="_blank" class="btn-outline-secondary btn">
                                        <font-awesome-icon icon="fa-brands fa-whatsapp"/>
                                        <span>{{ getLocale == 'EN' ? 'Chat Us' : 'Chat Kami' }}</span>
                                    </a>
                                    </template>
                                    <template v-if="ProductDetail.email">
                                    <a :href="`mailto:${ProductDetail.emailvalue}`" target="_blank" class="btn-outline-secondary btn">
...
export default {
    ...
    data() {
        return {
            ...
            image_url_detail: [
                    { id: '1', image_url: require('@/assets/images/product/health/detail/health_1.png') },
            ],
            ProductDetail: {
                type: '',
                product_name: '',
                ...
                composition: 'Caladine Powder Adult memiliki kandungan sulfur yang dapat menjadi anti bakteri alami serta salicylic acid juga untuk mengatasi bakteri. Setiap kemasan juga mengandung calamine sebagai anti pruritus, zinc oxide sebagai antiseptik, mentol untuk memberikan sensasi dingin, serta camphora untuk mengurangi gatal.',
                image_urls: [
                    { id: '1', image_url: require('@/assets/images/product/health/detail/health_3.png') },
                ]
            }
        }
    },
    methods: {
        ...
        getData (id) {
			this.$store.dispatch('product/getById', {id:id,status:'preview'}).then(data => {
