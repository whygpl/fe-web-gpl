                ...
                <div class="mt-8 grid grid-cols-1 gap-4 md:mt-10 md:grid-cols-2 md:gap-16">
                    ...
                    <div class="font-roboto">
                        <div class="w-full md:w-3/4">
                            <div class="line-clamp-3 text-2xl font-[900] text-white md:text-3xl">{{ getLocalNews.news[0]?.title }}</div>
                            ...
                            <div class="mt-4 line-clamp-4 text-sm font-[400] text-white md:mt-10 md:text-xl">{{ getLocale == 'EN' ? getLocalNews.news[0]?.desc_en : getLocalNews.news[0]?.desc }}</div>
                            <!-- <router-link v-for="item in NewsData" :to="{ path: '/cms/news/detail/preview/'+ item.id }"> -->
                            <router-link :to="{ path: '/cms/news/detail/preview/'+ getLocalNews.news[0]?.id }">
        ...
        <section class="w-full bg-white px-4 py-8 font-roboto md:px-0 md:py-10">
            <div class="container mx-auto">
                ...
                <div class="mt-4 grid grid-cols-2 gap-4 md:mt-6 md:grid-cols-3 md:gap-6">
                    <router-link v-for="item in getLocalNews.popular" :to="{ path: '/cms/news/detail/preview/'+ item.id }">
        ...
        <section class="w-full bg-gal-grey-light px-4 py-8 font-roboto md:px-4 md:py-16">
            <div class="container mx-auto">
                <div class="flex gap-2">
                    ...
                    <button v-for="item in getLocalNews.categorys" @click="filterNews(item.id)" class="btn" :class="item.id == active_filter ? 'btn-primary' : 'btn-outline-secondary'">{{ item.name }}</button>
                </div>
                <div class="mt-4 grid grid-cols-2 gap-4 md:mt-6 md:grid-cols-3 md:gap-6">
                    <router-link v-for="item in NewsData" :to="{ path: '/cms/news/detail/preview/'+ item.id }">
                        <NewsCard :title="item.title" :category="item.category" :date="item.date" :image_url="item.image_url" />
    ...
    methods: {
        ...
        getData () {
            if (localStorage.getItem('newspreview')) {
                this.$store.dispatch('news/get','preview').then(data => {
                    localStorage.setItem('newspreview', JSON.stringify(data))
                })
