    ...
    <div class="flex-1">
      ...
      <div class="bg-white p-8 m-4 min-h-screen rounded-box place-items-center">
        <div class="flex flex-col w-full">
          <div class="mb-10">
            <router-link :to="{ name: 'CmsNewsAdd' }"><button class="btn btn-primary">+ Tambah</button></router-link>
          </div>
          <div class="w-full">
            ...
            :pagination-options="{
              ...
              <template #table-row="props">
                <span v-if="props.column.field == 'actions'">
                  <router-link :to="{ path: '/cms/news/edit/'+ props.row.id}"><button class="btn btn-sm btn-primary">Edit</button></router-link>
                  <button @click="modalDelete(props.row.id)" class="btn btn-sm bg-error btn-primary">Hapus</button>
                  <router-link :to="{ path: '/news/detail/'+ props.row.id}"><button class="btn btn-sm btn-primary">Preview (Live)</button></router-link>
                </span>
                <span v-else>
...
export default {
  ...
  computed: {
    ...mapState({ 
      items: state => state.newscms.detail.items,
      columns: state => state.newscms.detail.columns,
      rows: state => state.newscms.detail.rows,
    }),
  },
  methods: {
    ...
    deleteRows () {
      this.$store.dispatch('newscms/newsDelete', {id: this.deleteId})
      .then(res => {
        this.getData()
        this.showModal = false
    ...
    getData () {
      this.$store.dispatch('newscms/getNews','preview').then(data => {
        data = data.map(d => {
            d.title = d.title?.replace(/<[^>]*>/g, '')
            d.title_en = d.title_en?.replace(/<[^>]*>/g, '')
            d.desc = d.desc?.replace(/<[^>]*>/g, '')
            d.desc_en = d.desc_en?.replace(/<[^>]*>/g, '')
            d.description = d.description?.replace(/<[^>]*>/g, '')
            d.description_en = d.description_en?.replace(/<[^>]*>/g, '')
            console.log(d);
