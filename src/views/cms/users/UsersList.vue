        ...
        <div class="flex-1">
            ...
            <div class="bg-white p-8 m-4 min-h-screen rounded-box place-items-center">
                <div class="flex flex-col w-full">
                    <div class="mb-10">
                        <router-link :to="{ name: 'CmsUsersAdd' }"><button class="btn btn-primary">+
                                Tambah</button></router-link>
                    </div>
                    <div class="w-full">
                ...
                infoFn: (params) => `Page ${params.firstRecordOnPage}`,
                            ...
                            <template #table-row="props">
                                <span v-if="props.column.field == 'actions'">
                                    <router-link :to="{ path: '/cms/users/edit/' + props.row.id }"><button
                                            class="btn btn-sm btn-primary">Edit</button></router-link>
                                    <button v-if="rows.length !== 1 " @click="modalDelete(props.row.id)"
                                        class="btn btn-sm bg-error btn-primary">Hapus</button>
                                </span>
                                <span v-else>
...
export default {
    ...
    computed: {
        ...mapState({
            items: state => state.users.usersList.items,
            columns: state => state.users.usersList.columns,
            rows: state => state.users.usersList.rows,
        }),
    },
    methods: {
        ...
        deleteRows() {
            this.$store.dispatch('users/usersDelete', { id: this.deleteId })
                .then(res => {
                    this.getData()
                    this.showModal = false
                },
        ...
        getData() {
            this.$store.dispatch('users/getUsers').then(data => {
                this.rows = data
    ...
    // created() {
    //     this.$store.dispatch('users/getDistributor')
    // },
    setup() {
