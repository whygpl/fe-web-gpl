<template>
  ...
  <div class="bg-base-300 flex min-h-screen">
    ...
    <div
      ...
      <Menu selectedMenu="Aboutus-Award" />
    </div>
    <!-- Main Content -->
    <div class="flex-1">
      <!-- Header Nav -->
      ...
      <div class="bg-white p-8 m-4 min-h-screen rounded-box place-items-center">
        <div class="flex flex-col w-full">
          <div class="w-full">
            ...
            <template v-for="(item, index) in form.item">
              <div class="flex w-full">
                <div class="form-control w-full">
                  <label class="label">
                    <span class="label-text"><b>Gambar {{ index + 1 }} | *244px x 244px | Maksimal 2 MB</b></span>
                    <div class="mt-2">
          ...
          <div class="mt-10">
            ...
            <router-link :to="{ name: 'AboutPreviewViewTab7' }"><button class="btn btn-neutral ml-2 hidden">Lihat
                Perubahan</button></router-link>
            <router-link :to="{ name: 'AboutViewTab7' }"><button class="btn btn-neutral ml-2">Lihat Perubahan
...
export default {
  ...
  components: { Navbar, Menu, Cropper,Footer, QuillEditor },
  data() {
    return {
      options: {
        debug: 'info',
  ...
  methods: {
    ...
    saveData() {
      ...
      this.$store.dispatch('aboutcms/formAwardPostPut', formData)
        .then(res => {
    ...
    getData(id) {
      this.$store.dispatch('aboutcms/getAward').then(data => {
  ...
  created() {
    this.$store.dispatch('aboutcms/getAward')
  },
  setup() {
