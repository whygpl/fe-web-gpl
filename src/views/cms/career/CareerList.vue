    ...
    <div class="flex-1">
      ...
      <div class="bg-white p-8 m-4 min-h-screen rounded-box place-items-center">
        <div class="flex flex-col w-full">
          <div class="mb-10">
            <router-link :to="{ name: 'CmsCareerAdd' }"><button class="btn btn-primary">+ Tambah</button></router-link>
          </div>
          <div class="w-full">
...
export default {
  ...
  computed: {
    ...mapState({ 
      items: state => state.careercms.list.items,
      columns: state => state.careercms.list.columns,
      rows: state => state.careercms.list.rows,
    }),
  },
  methods: {
    ...
    deleteRows () {
      this.$store.dispatch('careercms/joinDelete', {id: this.deleteId})
      .then(res => {
        this.getData()
        this.showModal = false
