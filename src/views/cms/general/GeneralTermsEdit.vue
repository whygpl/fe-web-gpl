<template>
  ...
  <div class="bg-base-300 flex min-h-screen">
    ...
    <div
      ...
      <Menu selectedMenu="General-Terms" />
    </div>
    <!-- Main Content -->
    <div class="flex-1">
      <!-- Header Nav -->
      ...
      <div class="bg-white p-8 m-4 min-h-screen rounded-box place-items-center">
        <div class="flex flex-col w-full">
          ...
          <div class="mt-10">
            ...
            <!-- <router-link :to="{ name: 'TermsConditionViewPreview' }"><button class="btn btn-neutral ml-2">Lihat Perubahan</button></router-link> -->
            <router-link :to="{ name: 'TermsConditionView' }"><button class="btn btn-neutral ml-2">Lihat Perubahan
...
export default {
  ...
  methods: {
    ...
    saveData() {
      ...
      this.$store.dispatch('generalcms/formTermsPostPut', formData)
        .then(res => {
          this.notification('Success :', 'You have successfully', 'bg-green-500', 'white', '50')
          if (localStorage.getItem('termslive')) {
            this.$store.dispatch('general/getExtra', { id: 9, status: 'live' }).then(data => {
              localStorage.setItem('termslive', JSON.stringify(data))
            })
          } else {
            this.$store.dispatch('general/getExtra', { id: 9, status: 'live' }).then(data => {
              localStorage.setItem('termslive', JSON.stringify(data))
          ...
          if (localStorage.getItem('termspreview')) {
            this.$store.dispatch('general/getExtra', { id: 9, status: 'preview' }).then(data => {
              localStorage.setItem('termspreview', JSON.stringify(data))
            })
          } else {
            this.$store.dispatch('general/getExtra', { id: 9, status: 'preview' }).then(data => {
              localStorage.setItem('termspreview', JSON.stringify(data))
    ...
    getData(id) {
      this.$store.dispatch('generalcms/getTerms').then(data => {
        console.log(data)
  ...
  created() {
    this.$store.dispatch('generalcms/getTerms')
  },
  setup() {
